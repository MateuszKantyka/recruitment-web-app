...
RSpec.describe InterestsQuery do
  describe '#call' do
    ...
    with name beginning with 'cosm' choosen by women aged between 20 and 30" do
      ...
      create(:interest, user_id: user_nr1.id, type: 'health', name: 'cosmos')
      create(:interest, user_id: user_nr1.id, type: 'health', name: 'cosmetics')
      user_nr2 = create(:user, is_male: false, birthday: Date.current - 22.year)
      create(:interest, user_id: user_nr2.id, type: 'health', name: 'cosmetics')
      create(:interest, user_id: user_nr2.id, type: 'work', name: 'ruby')
